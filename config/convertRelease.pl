...
#   Support automatic search for *.LDOBJS under <support-top>/dbd dirs
#
...
sub configAppInclude {
	...
	foreach $app (@includes) {
	    ...
	    print OUT "${app}_BIN = \$($app)/bin/$arch\n";
            # added bfr 2002-??-?? to support generation of .o dependencies
	    print OUT "SUPPORT_BIN += \$($app)/bin/$arch\n";
	}
	foreach $app (@includes) {
            $path = $macros{$app};
    ...
    # added bfr 2002-04-12:
    foreach $app (@includes) {
        $path = $macros{$app};
        next unless (-d "$path/dbd");
        print OUT "MAKEFLAGS.$arch += -I \$($app)/dbd\n";
    }
    close OUT;
}
